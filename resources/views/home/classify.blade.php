...
<div class="container menu-group">
    ...
    <div class="btn-group navlist" role="group" aria-label="...">
        <ul class="read-nav clearfix" style="width: 1165px;">

            <li class="navlist_li"><a class="btn mmen" style="font-size: 18px;" href="{{asset(url('home/index'))}}">首页</a></li>
            <li class="navlist_li"><a class="btn mmen" style="font-size: 18px;" href="{{asset(url('home/category'))}}">分类</a></li>
            ...
            <span style="margin-left: 480px;" class="iconfont bookshelf">&#xe618;<a href="{{asset(url('home/user/center'))}}" class="btn">我的书架</a></span>
        </ul>
    </div>
    <script>
...
<div class="kePublic">
    ...
    <div class="content">
        <ul class="vertical-nav dark red">
            @foreach($data as $v)
                <li class="active"><a class="ziti" href="{{asset(url('home/category',$v->id))}}">{{$v->name}}</a>
                    <ul>
                        @forelse($ddat as $data)
                            @if($ddat[0]->pid == $v->id)

                                    <li><a href="{{asset(url('home/category/secend',$data->id))}}">{{$data->name}}</a></li>
                            @endif
                        @empty
                        @endforelse
