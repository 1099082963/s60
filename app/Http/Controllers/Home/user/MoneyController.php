...
//        dd($list);
    ...
    public function core(){
        ...
        $getcore = $core[0]->all;
        $l =DB::select("select sum(`dismoney`) as 'all' from core where uid=".$id);
        $dismoney = $l[0]->all;
        $c =DB::select("select sum(`discore`) as 'all' from core where uid=".$id);
        $discore = $c[0]->all;
        return view('home.center.Money.core')
            ->with('money',$money)
